...
class Jeu:
    ...
    #Sauvegarde certaine donné dans un fichier
    def save(self):
        with open("save.txt","w") as f:
            f.write(str(self.joueur1.coord)+"\n")
            f.write(str(self.joueur2.coord2)+"\n")
            f.write(str(self.joueur1.score1)+"\n")
            f.write(str(self.joueur2.score2)+"\n")
            
    #Charger les données du fichier dans les variables du jeu      
    def load(self): 
        p = 0      
        t = "\n"
        res = []
        ...
        for n in res:
            ...
            for x in range(len(t)):
                string = re.sub("\n","",n)
            if p == 1:
                self.joueur1.coord = int(string)
            if p == 2:
                self.joueur2.coord2 = int(string)
            if p == 3:
                self.joueur1.score1 = int(string)
            if p == 4:
                self.joueur2.score2 = int(string)
